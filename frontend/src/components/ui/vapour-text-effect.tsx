...
const renderCanvas = ({
  ...
  let fontSize = 50;
  const rawFontSize = framerProps.font?.fontSize || "50px";
  
  if (typeof rawFontSize === "string") {
    if (rawFontSize.includes("clamp")) {
      // Approximate clamp for canvas (assume middle value for now or calculate)
      fontSize = Math.floor(width * 0.08); // 8vw approximate
    } else if (rawFontSize.includes("vw")) {
      fontSize = Math.floor(parseFloat(rawFontSize) * (width / 100));
    } else if (rawFontSize.includes("rem")) {
      fontSize = parseFloat(rawFontSize) * 16;
    } else {
      fontSize = parseInt(rawFontSize.replace("px", "")) || 50;
    }
  } else {
  ...
  const font = `${framerProps.font?.fontWeight ?? 900} ${fontSize * globalDpr}px ${framerProps.font?.fontFamily ?? "Outfit, sans-serif"}`;
