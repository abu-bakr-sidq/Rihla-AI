...
export function DottedSurface({ className, ...props }: DottedSurfaceProps) {
	...
	useEffect(() => {
		...
		const AMOUNTX = 30;  // was 40 → reduced from 2400 to 900 particles
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
				...
				if (theme === 'dark') {
					colors.push(0.8, 0.8, 0.8); // 200/255 approx
				} else {
					colors.push(0, 0, 0);
				}
			}
		...
		// Pause animation when tab is hidden (saves CPU when user switches tabs)
