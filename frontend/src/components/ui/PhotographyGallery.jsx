...
function FallbackGallery({ images }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-slate-900 p-4">
            <p className="text-slate-400 mb-4 text-sm font-mono uppercase tracking-widest">WebGL not supported. Asset preview mode.</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto p-4 bg-slate-800/50 rounded-xl backdrop-blur-xl border border-white/10">
                {normalizedImages.map((img, i) => (
                    <img key={i} src={img.src || '/placeholder.svg'} alt={img.alt} className="w-full h-32 object-cover rounded-lg border border-white/5 shadow-2xl" />
                ))}
            </div>
        </div>
    );
    ...
    return (
        <div className={className} style={{ ...style, background: 'transparent' }}>
            <Canvas camera={{ position: [0, 0, 0], fov: 55 }} gl={{ antialias: true, alpha: true }}>
                <Suspense fallback={<Html center><div className="text-[#D4AF37] font-black text-[10px] tracking-widest uppercase opacity-40">Initializing 3D Realm...</div></Html>}>
